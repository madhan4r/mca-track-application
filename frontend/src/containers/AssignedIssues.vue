    ...
    <div class="col-lg-10">
      <div class="card mt-3">
        <div class="card-body text-center">
          <h4 class="card-title m-b-0">Assigned Issues</h4>
        </div>
...
export default {
  ...
  data: () => ({
    ...
    paginationControl: {},
    reqFilters: ["status_id__in", "type_id__in"],
    Roles
  }),
  computed: {
    ...mapGetters([
  ...
  methods: {
    ...
    async fetchIssues() {
      ...
      if (!isEmptyObjectCheck(rest)) {
        await this.fetchFilteredIssuesCount({
          assigned_to__in: this.getUserId,
          ...rest
        });
  ...
  async mounted() {
    await this.fetchIssueCounts({ assigned_to__in: this.getUserId });
    this.fetchIssues();
  }
};
</script>
